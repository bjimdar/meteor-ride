...
namespace MeteorRide_IntegrationTests
{
	...
	public class ShellMenuItemTest
	{
		...
		public void LaunchCommand()
		{
			...
			{
				CommandID menuItemCmd = new CommandID(Vectria.MeteorRide.GuidList.guidMeteorRideCmdSet, 
					(int)Vectria.MeteorRide.PkgCmdIDList.cmdIdMeteorExplorer);

				// Create the DialogBoxListener Thread.
				string expectedDialogBoxText = string.Format(CultureInfo.CurrentCulture, 
					"{0}\n\nInside {1}.MenuItemCallback()", 
					"Meteor Tools for Visual Studio", 
					"Vectria.MeteorPackage.MeteorPackage");
